      ...
      if (jsonBody) {
        if (res.status === 200) {
          ...
          if (link.type === AlbumType.album) {
            ...
            // Preserve the album thumbnail asset ID if provided
            if (album.albumThumbnailAssetId && link.album) {
              link.album.albumThumbnailAssetId = album.albumThumbnailAssetId
            }
          }
